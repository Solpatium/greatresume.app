import React from "react";
import cn from "classnames";

const colors = {
  danger: "bg-pink-600 hover:bg-pink-700 focus:ring-pink-500 text-white ",
  primary: "bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-500 text-white",
  secondary: "bg-white hover:bg-gray-50 border-gray-300 focus:ring-indigo-500 text-gray-700",
};

export const Button: React.FC<{
  danger?: boolean;
  secondary?: boolean;
  onClick?: () => void;
  type?: "button" | "submit";
  icon?: (props: React.ComponentProps<"svg">) => JSX.Element;
}> = ({ danger, onClick, secondary, type, children, icon }) => {
  const variant = (danger && colors.danger) || (secondary && colors.secondary) || colors.primary;
  return (
    <button
      onClick={onClick}
      type={type ?? "button"}
      className={cn(
        "inline-flex justify-center items-center px-4 py-2 border   text-sm font-medium rounded shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2",
        variant,
      )}>
      {icon && React.createElement(icon, { className: "h-4 w-4 mr-2 -ml-0.5" })}
      {children}
    </button>
  );
};
